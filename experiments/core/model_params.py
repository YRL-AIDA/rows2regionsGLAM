...
def node_classifier(in_dim, num_classes, hidden_sizes=None):
    if hidden_sizes is None:
        hidden_sizes = [256, 128]
    return mlp(in_dim, hidden_sizes, num_classes, activation="gelu", last_activation="none")


def edge_classifier(in_dim, hidden_sizes=None):
    if hidden_sizes is None:
        hidden_sizes = [256, 64]
...
def default_arch(
    ...
    params = {
        ...
        "node_classifier_block": {
            "linear_post": node_classifier(concat_node_out, num_classes),
        },
        "edge_classifier_block": {
            "linear_post": edge_classifier(2 * (hidden_dim + input_dim) + edge_dim)
            if has_post_node
            else edge_classifier(2 * (concat_node_out + input_dim) + edge_dim),
        },
        "epochs": epochs,
