...
class RowGLAMTokenizer(BaseTokenizer):
    def get_name(self):
        return "RowGLAM norm geometry"
    
    def __call__(self, rows_json, h, w):
        A = self.get_A(rows_json)
        node_features = self.get_node_features(rows_json, h, w)
        edge_features = self.get_edge_features(A, rows_json, h, w)
        json_info =  {
            'A': A,
    ...
    def get_node_features(self, rows_json, h, w):
        if len(rows_json) == 0:
            return [[]]
        ...
        coord_vec = np.array([self.get_vec_coord(r_json, norm_h=h, norm_w=w) for r_json in rows_json])
    ...
    def get_edge_features(self, A, rows_json, h, w):
        ...
        for i, j in zip(A[0], A[1]):
            ...
            edges_featch.append([r1.get_angle_center(r2), r1.get_min_dist(r2), abs(x1-x2)/w, abs(y1-y2)/h])
        # print(edges_featch)
        return edges_featch
