...
class Tester:
    ...
    def calculate_target_and_preds(self, test_dataset, name_dataset, name_test_dataset, dataset_path, test_path):
        ...
        for i, d in enumerate(test_dataset):
            ...
            clean_bboxes = self.clean_bboxes_true([reg['segment'] for reg in true_regions])

            bboxes_true = []
            classes_true = []

            for reg in true_regions:
                if reg['segment'] in clean_bboxes:
                    bboxes_true.append(reg['segment'])
                    classes_true.append(reg['category_id'])
