from rows2regionsGLAM._page_model import BaseConverter, RegionModel, RowsModel, MergeExtractor
...
class Rows2Regions(BaseConverter):
    def __init__(self, conf):
        self.rows2regionsGLAM_tokenizer = conf['tokenizer']
    ...
    def regions_from_graph(self, rows_json, graph, deleted_edges, node_classes):
        ...
        for reg in graph_.get_related_graphs():
            ...
            row_classes  = np.array([node_classes[i].detach().cpu().numpy() for i in indexes])
